...
with open('1iter_graphsage.json',) as f:
  route = json.load(f)
...
for i in range(len(route)):
  hb_name = "graphsage_hb_%d" % i
  name = "graphsage_data_new_format_%d" % i
  # work dir
  sh_cmd = "mkdir -p " + name
  print(sh_cmd)
  os.system(sh_cmd)
  for j in range(2):
    xeon_name = "graphsage_xeon_%d_%d" % (i, j)
    # collect full stack
  ...
  sh_cmd = "(cd " + hb_name + "; python /work/shared/common/research/bladerunner-v4.3/bsg_manycore/software/py/vanilla_parser/stats_parser.py --stats vanilla_stats.csv --vcache-stats vcache_stats.csv --tile)"
  print(sh_cmd)
  os.system(sh_cmd)
  # collect output
  ...
  # ======================================================
  full = ["{0}/full-{1:02d}.std".format(name, j) for j in range(2)]
  chunk = ["{0}/full-{1:02d}.std".format(name, j) for j in range(2)]
