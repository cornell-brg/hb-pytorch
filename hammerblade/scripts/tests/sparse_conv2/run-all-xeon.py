...
for i in range(len(route)):
  for j in range(20):
    ...
    fancy_print(cmd)
    print()
    name = "sparse_conv2_xeon_%d_%d" % (i, j)
    sh_cmd = "mkdir -p " + name
    print(sh_cmd)
    os.system(sh_cmd)
   ...
   # print(sh_cmd)
   # os.system(sh_cmd)
    script = "(cd " + name + "; python -m pytest -vs /scratch/users/zz546/pytorch-cosim/hb-pytorch/hammerblade/torch/tests/profiler/test_lenet5_sparseconv2_profile_route.py > out.std 2>&1)"
