...
for i in range(len(route)):
  for j in range(20):
    ...
    fancy_print(cmd)
    print()
    name = "dense_mm_xeon_%d_%d" % (i, j)
    sh_cmd = "mkdir -p " + name
    print(sh_cmd)
    os.system(sh_cmd)
    with open(name + "/dense_mm.json", 'w') as outfile:
...
#    os.system(sh_cmd)
   ...
   # print(sh_cmd)
   # os.system(sh_cmd)
    script = "(cd " + name + "; python -m pytest -vs /scratch/users/zz546/pytorch-cosim/hb-pytorch/hammerblade/torch/tests/profiler/test_dense_mm.py > out.std 2>&1)"
