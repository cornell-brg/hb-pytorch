...
def _test_torch_nn_NLLLoss(loss, m, input, target):
...
def test_torch_nn_NLLLoss_mean():
    ...
    _test_torch_nn_NLLLoss(loss, m, input, target)
...
def _test_torch_nn_NLLLoss_weight(loss, loss_h, m, input, target):
    output = loss(m(input), target)
...
def test_torch_nn_NLLLoss_mean_weight():
    ...
    input = torch.randn(3, 5, requires_grad=True)
    target = torch.tensor([1, 0, 4])
    _test_torch_nn_NLLLoss_weight(loss, loss_h, m, input, target)
