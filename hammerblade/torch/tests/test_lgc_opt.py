...
#    indices = torch.LongTensor([coo_data.row, coo_data.col])
    ...
    t = t.coalesce()
    c2sr_cpu= t.convert_format()
    c2sr_hb = c2sr_cpu.hammerblade()
    return c2sr_hb
...
#    print(c2sr.size(0))
    d_sqrt = torch.from_numpy(d_sqrt).float().hammerblade()
    dn_sqrt = torch.from_numpy(dn_sqrt).float().hammerblade()
...
#    print(d_sqrt.size(0))
    ...
    for seed in tqdm(seeds):
        s = np.zeros(adj.shape[0])
        s[seed] = 1
        s = torch.from_numpy(s).float().hammerblade()
        q = zero   
        rad = rho * alpha * d_sqrt
        grad0 = -alpha * dn_sqrt * s
        grad = grad0.clone()
        for _ in range(iters):
            ...
            grad = grad0 + torch.mv_xcel(c2sr, q)
            print("Finish spmv xcel")
