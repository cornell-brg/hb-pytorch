...
def torch_ista(seeds, adj, size, alpha, rho, iters):
    out = torch.empty(0)
    for seed in tqdm(seeds):
        s = np.zeros(adj.shape[0])
        s[seed] = 1
        ...
        Q = torch.from_numpy(Q).to_sparse().float().hammerblade()
        q = torch.zeros(size, 1).hammerblade()
        d_sqrt = torch.from_numpy(d_sqrt).float()
        dn_sqrt = torch.from_numpy(dn_sqrt).float()
        s = torch.from_numpy(s).float()
        
        rad = rho * alpha * d_sqrt
        rad = rad.view(size, 1).hammerblade()
        grad0 = -alpha * dn_sqrt * s
        grad0 = grad0.view(size, 1).hammerblade()
        grad = grad0.clone().hammerblade()
        d_sqrt = d_sqrt.view(size, 1).hammerblade()
        zero = torch.zeros(size, 1).hammerblade()
        for _ in range(iters):
            q = torch.max(q - grad - rad, zero)
            grad = grad0 + torch.mm(Q, q)
...
if __name__ == "__main__":
    ...
    t1 = time()
    torch_out = torch_ista(ista_seeds, adj, size, alpha=args.alpha, rho=args.ista_rho, iters=args.ista_iters)
