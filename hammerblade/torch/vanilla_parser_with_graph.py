...
#
#   {print_stats}     Print performance stats to terminal (for microbenchmarking)
#   {graph}           Produce performance metric graphs
...
class VanillaStatsParser:
    ...
    def print_exec_time_stacks_graph(self, graph, print_stats):
        barsize = 0.5
        ...
        for tag in self.tile_stat:
            ...
            for tile in self.tile_stat[tag]:
                ...
                for k in self.instrs:
                    if not k in type_instr_cnt:
                        type_instr_cnt[k] = 0
                    type_instr_cnt[k] = ctr[k]

                tiles.append(tile)
                tile_data_other.append(ctr['cycle'] - ctr['stall_total'] - ctr['bubble_total'])
            ...
            if graph:
                ...
                # graph other
                ax.bar(tile_inds, tile_data_other, color='black', width=barsize)
            ...
            if print_stats:
                print('tile_data_stall')
                pprint.pprint(tile_data_stall)
                print('tile_data_bubble')
                pprint.pprint(tile_data_bubble)
                print('tile_data_other')
                pprint.pprint(tile_data_other)
                print('type_instr_cnt')
                pprint.pprint(type_instr_cnt)
...
def parse_args():
    ...
    parser.add_argument("--print_stats", default=False, action='store_true',
                        help="Print performance stats (for microbenchmarking).")
...
if __name__ == "__main__":
    ...
    # graph and/or print stats for microbenchmarking
    st.print_exec_time_stacks_graph(args.graph, args.print_stats)
