...
inline void dram_to_sp_simple(
    ...
    for (int i = 0; i < BLOCK_DIM; i++) {
        ...
        if (BLOCK_DIM == 12) {
            ...
            register float tmp10 = *(src_offset + 10);
            register float tmp11 = *(src_offset + 11);
            register float tmp12 = *(src_offset + 12);
            register float tmp13 = *(src_offset + 13);
            register float tmp14 = *(src_offset + 14);
            register float tmp15 = *(src_offset + 15);
            asm volatile("": : :"memory");
            dest[row_offset + 0]  = tmp0;
            dest[row_offset + 1]  = tmp1;
            dest[row_offset + 2]  = tmp2;
            ...
            dest[row_offset + 10] = tmp10;
            dest[row_offset + 11] = tmp11;
            dest[row_offset + 12] = tmp12;
            dest[row_offset + 13] = tmp13;
            dest[row_offset + 14] = tmp14;
            dest[row_offset + 15] = tmp15;
            row_offset += 16;
        }
...
inline void dram_to_sp_simple_generic(
    ...
    for (int i = 0; i < BLOCK_DIM; i++) {
        ...
        if (BLOCK_DIM == 12) {
            ...
            register float tmp10 = *(src_offset + 10 * src_strides[1]);
            register float tmp11 = *(src_offset + 11 * src_strides[1]);
            register float tmp12 = *(src_offset + 12 * src_strides[1]);
            register float tmp13 = *(src_offset + 13 * src_strides[1]);
            register float tmp14 = *(src_offset + 14 * src_strides[1]);
            register float tmp15 = *(src_offset + 15 * src_strides[1]);
            asm volatile("": : :"memory");
            dest[row_offset +  0] = tmp0;
            dest[row_offset +  1] = tmp1;
            dest[row_offset +  2] = tmp2;
