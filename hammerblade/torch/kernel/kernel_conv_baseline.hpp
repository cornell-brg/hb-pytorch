...
struct Unroll {
  ...
  inline static void fill_buffer_rotate(T* src, T* buf);
...
template<int DIM>
inline void fill_filter_buffer_rotate(float* src, float* buf) {
  Unroll<DIM*DIM-1, float>::fill_buffer_rotate(src, buf);
}
...
inline void conv2d_5x5_back(float* imap, float* filter, float* omap) {
  for (size_t y = 0; y < BLOCK_DIM_Y; y++) {
    for (size_t x = 0; x < BLOCK_DIM_X; x += 8) {
      ...
      for (size_t yy = 0; yy < FILTER_DIM; yy++) {
        ...
        register float imap9  = imap[y * IMAP_DIM_X + x + yy * IMAP_DIM_X + 9];
        register float imap10 = imap[y * IMAP_DIM_X + x + yy * IMAP_DIM_X + 10];
        register float imap11 = imap[y * IMAP_DIM_X + x + yy * IMAP_DIM_X + 11];
        asm volatile("": : :"memory");
...
template<int B_DIM_X, int B_DIM_Y, int I_DIM_X, int I_DIM_Y, int F_DIM>
inline void conv2d_1x1_16(float* imap, float* filter, float* omap) {
    for (size_t y = 0; y < B_DIM_Y; ++y) {
        for (size_t x = 0; x < B_DIM_X; x += 8) {
            register float psum0 = omap[y * B_DIM_X + x  + 0];
            register float psum1 = omap[y * B_DIM_X + x  + 1];
            ...
            register float imap0 = imap[y * B_DIM_X + x + 0];
            register float imap1 = imap[y * B_DIM_X + x + 1];
            register float imap2 = imap[y * B_DIM_X + x + 2];
            register float imap3 = imap[y * B_DIM_X + x + 3];
            register float imap4 = imap[y * B_DIM_X + x + 4];
            register float imap5 = imap[y * B_DIM_X + x + 5];
            register float imap6 = imap[y * B_DIM_X + x + 6];
            register float imap7 = imap[y * B_DIM_X + x + 7];
            asm volatile("": : :"memory");
...
template<int B_DIM_X, int B_DIM_Y, int I_DIM_X, int I_DIM_Y, int F_DIM>
inline void conv2d_1x1_4(float* imap, float* filter, float* omap) {
    for (size_t y = 0; y < B_DIM_Y; ++y) {
        for (size_t x = 0; x < B_DIM_X; x += 4) {
            register float psum0 = omap[y * B_DIM_X + x  + 0];
            register float psum1 = omap[y * B_DIM_X + x  + 1];
