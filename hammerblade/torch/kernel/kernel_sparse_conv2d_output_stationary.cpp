...
extern "C" {
  ...
  __attribute__ ((noinline))  int tensorlib_sparse_convolution_forward(
    ...
    size_t len_per_tile = std::ceil((float)Cout / (float)(bsg_tiles_X * bsg_tiles_Y));
    ...
    if(end > start) {
      int32_t nnz = w_row(end) - w_row(start);
      int32_t wcol[nnz];
      float wval[nnz];
      int32_t wrow[end - start + 1];
      for(uint32_t c = start; c <= end; c++) {
        wrow[c - start] = w_row(c);
      }
      
      int32_t offset = wrow[0];
     
      for(uint32_t in = 0; in < nnz; in++) {
        wcol[in] = w_col(in+offset);
        wval[in] = w_val(in+offset);
      }
        ...
        for(uint32_t co = start; co < end; ++co) {
          for(uint32_t yh = 0; yh < Hout; ++yh) {
            for(uint32_t yw = 0; yw < Wout; ++yw) {
              ...
              for(uint32_t i = wrow[co - start]; i < wrow[co - start + 1]; i++) {
                int32_t index = i - offset;
