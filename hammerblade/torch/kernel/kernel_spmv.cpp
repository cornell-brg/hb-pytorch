...
//    printf("Enter the spmv kernel\n");
    ...
    for (uint32_t i = start; i < end; i = i + thread_num) {
      float temp = 0.0;
      for(uint32_t idx = c2sr(i); idx < c2sr(i) + csr(i + 1) - csr(i); idx++) { //C2SR MODE
...
//       printf("indices(%d) is %d\n", col_index, indices(col_index));
  ...
  HB_EMUL_REG_KERNEL(tensorlib_spmv, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*)
}
