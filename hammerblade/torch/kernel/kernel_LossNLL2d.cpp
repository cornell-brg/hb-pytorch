...
__attribute__ ((noinline))
int tensorlib_nll_loss2d_backward(
  ...
  const float grad_output_value  = *(float*)grad_output.data_ptr();
  const int64_t* target_data     = (int64_t*)target.data_ptr();
  float* grad_input_data         = (float*)grad_input.data_ptr();
  const float total_weight_value = *(float*)total_weight.data_ptr();

  const uint32_t batch_size  = input.dim(0);
  const uint32_t n_classes   = input.dim(1);
  const uint32_t map_size    = input.dim(2) * input.dim(3);
  const uint32_t sample_size = map_size * n_classes;
  ...
  float normalize = (reduction == Reduction::Mean)
      ? total_weight_value : static_cast<float>(1);

  for (size_t b = 0; b < batch_size; b++) {
    hb_tiled_for(map_size, [&](size_t elem) {
      const uint32_t cur_target = static_cast<uint32_t>(target_data[b * map_size + elem]);

      // here we assume weight is 1.0f
      if (cur_target != ignore_index) {
        hb_assert(cur_target >= 0 && cur_target < n_classes);

        const uint32_t index = b * sample_size + cur_target * map_size + elem;
        grad_input_data[index] = -1.0f / normalize * grad_output_value;
      }
    });
  }
...
HB_EMUL_REG_KERNEL(tensorlib_nll_loss2d_backward, hb_tensor_t*, hb_tensor_t*,
                   hb_tensor_t*, hb_tensor_t*, hb_tensor_t*, uint32_t*, int32_t*)
