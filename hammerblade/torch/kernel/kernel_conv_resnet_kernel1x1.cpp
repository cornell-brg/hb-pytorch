...
extern "C" {
  ...
  __attribute__ ((noinline))  int tensorlib_conv_resnet_32_1x1_32x32(
    hb_tensor_t* output,
    hb_tensor_t* input,
    hb_tensor_t* weight,
    hb_vector_t* padding,
    ...
    hb_assert(PADDING == p[0]); // assume padding == 0
    hb_assert(PADDING == p[1]);
    hb_assert(STRIDE == s[0]); // assume stride == 1
    hb_assert(STRIDE == s[1]);
    ...
    for (size_t idx = bsg_id / 2; idx < num_blocks; idx += (BSG_TILE_GROUP_X_DIM * BSG_TILE_GROUP_Y_DIM)/2) {
      if (bsg_id % 2 == 0 && idx < num_blocks) {
        ...
        for (size_t channel_id = 0; channel_id < Cin; channel_id++) {
          ...
          // do conv
          conv2d_1x1_16<BLOCK_DIM_X, BLOCK_DIM_Y, IMAP_DIM_X, IMAP_DIM_Y, FILTER_DIM>(imap_buf, filter_buf, omap_buf);
  ...
  __attribute__ ((noinline))  int tensorlib_conv_resnet_32_1x1_32x32_back_input(
    hb_tensor_t* output,
    hb_tensor_t* input,
    hb_tensor_t* weight,
    hb_vector_t* padding,
  ...
  __attribute__ ((noinline))  int tensorlib_conv_resnet_32_1x1_32x32_back_weight(
    hb_tensor_t* output,
    hb_tensor_t* input,
    hb_tensor_t* weight,
    hb_vector_t* padding,
              ...
              for (size_t f_y = 0; f_y < FILTER_DIM; f_y++) {
                ...
                for (size_t y = 0; y < BLOCK_DIM_Y; y++) {
                  ...
                  for (size_t x = 0; x < BLOCK_DIM_X; x += 8) {
                    ...
                    psum += imap7 * grad7;
#else
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap0), "f"(grad0));
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap1), "f"(grad1));
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap2), "f"(grad2));
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap3), "f"(grad3));
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap4), "f"(grad4));
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap5), "f"(grad5));
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap6), "f"(grad6));
                    asm volatile("fmadd.s %0, %1, %2, %0" : "+f"(psum) : "f"(imap7), "f"(grad7));
#endif
                  }
                  imap_ptr += IMAP_DIM_X;
  ...
  HB_EMUL_REG_KERNEL(tensorlib_conv_resnet_32_1x1_32x32, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*,
                     hb_vector_t*, hb_vector_t*)

  HB_EMUL_REG_KERNEL(tensorlib_conv_resnet_32_1x1_32x32_back_input, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*,
                     hb_vector_t*, hb_vector_t*)

  HB_EMUL_REG_KERNEL(tensorlib_conv_resnet_32_1x1_32x32_back_weight, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*,
                     hb_vector_t*, hb_vector_t*)
