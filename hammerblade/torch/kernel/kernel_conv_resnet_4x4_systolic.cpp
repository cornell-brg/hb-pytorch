...
inline void imapDMA_padding_systolic(HBTensor<float, 4>& imap, float* imap_buf, size_t image_id, size_t channel_id, size_t block_x, size_t block_y) {
  ...
  // 4x4 case there are 16 elements -- fully unroll
  ...
  imap_buf[buf_offset + 0]  = tmp04;
  imap_buf[buf_offset + 1]  = tmp05;
  imap_buf[buf_offset + 2]  = tmp06;
  imap_buf[buf_offset + 3]  = tmp07;
  buf_offset += IMAP_DIM_X;
  imap_buf[buf_offset + 0]  = tmp08;
  imap_buf[buf_offset + 1]  = tmp09;
  imap_buf[buf_offset + 2] = tmp10;
  imap_buf[buf_offset + 3] = tmp11;
  buf_offset += IMAP_DIM_X;
  imap_buf[buf_offset + 0] = tmp12;
  imap_buf[buf_offset + 1] = tmp13;
  imap_buf[buf_offset + 2] = tmp14;
...
extern "C" {
  ...
  __attribute__ ((noinline))  int tensorlib_conv_resnet_32_3x3_4x4_systolic(
    hb_tensor_t* output,
    hb_tensor_t* input,
    hb_tensor_t* weight,
    hb_vector_t* padding,
    ...
    auto imapDMA_job = [&]() {
      ...
      size_t block_x = 0; // block_x can only be 0
      size_t block_y = 0; // block_y can only be 0
      size_t image_offset = bsg_y;

      // we can do 8 images across 8 rows
      for (size_t image_id = image_offset; image_id < N; image_id+=8) {
        ...
        for (size_t filter_id = filter_offset; filter_id < Cout; filter_id += 15) {
          if (filter_id < Cout) {
            ...
            for (size_t channel_id = 0; channel_id < Cin; channel_id++) {
              ...
              // do conv
              conv2d_3x3_4<BLOCK_DIM_X, BLOCK_DIM_Y, IMAP_DIM_X, IMAP_DIM_Y, FILTER_DIM>(imap_buf, filter_buf, omap_buf);
  ...
  __attribute__ ((noinline))  int tensorlib_conv_resnet_32_3x3_4x4_back_input_systolic(
    hb_tensor_t* output,
    hb_tensor_t* input,
    hb_tensor_t* weight,
    hb_vector_t* padding,
  ...
  __attribute__ ((noinline))  int tensorlib_conv_resnet_32_3x3_4x4_back_weight_systolic(
    hb_tensor_t* output,
    hb_tensor_t* input,
    hb_tensor_t* weight,
    hb_vector_t* padding,
                ...
                for (size_t block_x = 0; block_x < w_blocks_per_out_channel; block_x++) {
                  ...
                  for (size_t f_y = 0; f_y < FILTER_DIM; f_y++) {
                    ...
                    for (size_t y = 0; y < BLOCK_DIM_Y; y++) {
                      ...
                      for (size_t x = 0; x < BLOCK_DIM_X; x += 4) {
  ...
  HB_EMUL_REG_KERNEL(tensorlib_conv_resnet_32_3x3_4x4_systolic, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*,
                     hb_vector_t*, hb_vector_t*)

  HB_EMUL_REG_KERNEL(tensorlib_conv_resnet_32_3x3_4x4_back_input_systolic, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*,
                     hb_vector_t*, hb_vector_t*)

  HB_EMUL_REG_KERNEL(tensorlib_conv_resnet_32_3x3_4x4_back_weight_systolic, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*,
                     hb_vector_t*, hb_vector_t*)
