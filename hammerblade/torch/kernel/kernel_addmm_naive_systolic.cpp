...
inline void spcpy_bias(bsg_attr_remote float* bsg_attr_noalias dest, float* src) {
...
extern "C" {
  ...
  __attribute__ ((noinline))  int tensorlib_addmm_naive_systolic(
          hb_tensor_t* _result,
          hb_tensor_t* _self,
          hb_tensor_t* _mat1,
          hb_tensor_t* _mat2) {
    ...
    auto compute_task = [&] () {
      ...
      for (int rr = bsg_y-1; rr < m1_num_blk_per_col; rr += SYSTOLIC_Y_DIM) {
        for (int rc = bsg_x-1; rc < m2_num_blk_per_row; rc += SYSTOLIC_X_DIM) {
          ...
          // reuse mat2_fifo to pass *input* matrix, which is a row vector
          sp_mat2 = mat2_fifo.obtain_rd_ptr();
          if (should_pass_down && rr+1 < m1_num_blk_per_col) {
            sp_mat2_remote = mat2_fifo.obtain_wr_ptr();
            // copy mat2 to S
            spcpy_bias(sp_mat2_remote, sp_mat2);
            mat2_fifo.finish_wr_ptr();
          }

          // write back
          addmm_and_sp_to_dram_naive(result, sp_result, sp_mat2, rr, rc);
          mat2_fifo.finish_rd_ptr();
        }
      }
    };
    ...
    auto col_dma_task = [&] () {
      for (int i = 0; i < m1_num_blk_per_col; i += SYSTOLIC_Y_DIM) {
        for (int rc = bsg_x-1; rc < m2_num_blk_per_row; rc += SYSTOLIC_X_DIM) {
          ...
          // load *bias*
          sp_mat2_remote = mat2_fifo.obtain_wr_ptr();
          load_bias(sp_mat2_remote, self, i, rc);
          mat2_fifo.finish_wr_ptr();
  ...
  HB_EMUL_REG_KERNEL(tensorlib_addmm_naive_systolic, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*, hb_tensor_t*)
