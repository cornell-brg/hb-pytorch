...
Tensor stddtmmt_hb(const SparseTensor& sample, const Tensor& b, const Tensor& c) {
  ...
  TORCH_CHECK(b.size(0) == sample.size(1) && c.size(0) == sample.size(0),"STddTmmT sample dimension mismatch: sample.T was shape ",sample.size(1)," by ",sample.size(0),", but (b@c.T) is shape ",b.size(0)," by ",c.size(0));
