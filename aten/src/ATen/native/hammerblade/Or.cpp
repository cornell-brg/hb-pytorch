...
Tensor or_kernel_hb(const Tensor& self, const Tensor& other) {
  ...
  TORCH_CHECK(other.scalar_type() == kInt || other.scalar_type() == kBool, "HammerBlade or is implemented for Int and Bool only");
  Tensor result = at::empty_like(self, self.options());
  if (self.scalar_type() ==kInt) {
    hb_offload_kernel(result, self, other, "tensorlib_or_int");
  }
  else {
    hb_offload_kernel(result, self, other, "tensorlib_or_bool");
  }
  return result;
}
